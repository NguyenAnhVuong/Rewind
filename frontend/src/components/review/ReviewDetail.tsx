...
const ReviewDetail = (props: Props) => {
  ...
  useEffect(() => {
    const getDetailReviewFromApi = async () => {
      try {
        const res = await axios.get(`/api/restaurant/details/${id}`);
        console.log("res: ", res);
        if (res.data.status) {
          setImages([
            res.data.data.restaurant.avatar,
            ...res.data.data.restaurant.images,
          ]);
          setComments(res.data.data.restaurant.comments);
        }
      } catch (e) {
        console.log("e: ", e);
      }
